        ...
        public void Render(double interval)
        {
            ...
            if (!inited)
            {
                ...
                for (int i = 0, j = 0; i < net; i += step, j++)
                {
                    ...
                    RenderState state = new RenderState(fbufs[j], new ShaderProgram(ShaderSource.Load(ShaderType.VertexShader, "Graphics/OpenGL/Shaders/Framebuffer/vertex.glsl"), ShaderSource.Load(ShaderType.FragmentShader, "Graphics/OpenGL/Shaders/HeightMapGI_A/fragment.glsl")), null, new UniformBuffer[] { v_dash }, false, DepthFunc.None, 0, 1, BlendFactor.SrcAlpha, BlendFactor.OneMinusSrcAlpha, Vector4.Zero, 1, CullFaceMode.Back);
